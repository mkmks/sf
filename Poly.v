...
None. (non-well-founded induction)
...
Fixpoint split
  ...
  match l with
    | [] => ([], [])
    | (x, y) :: xys => let (xs, ys) := split xys in
                       (x :: xs, y :: ys)
end.
...
Definition partition {X : Type} (test : X -> bool) (l : list X)
                     : list X * list X :=
  let xs := filter test l in
  let ys := filter (fun x => negb (test x)) l in (xs, ys).
...
Lemma map_snoc : forall (X Y : Type) (f : X -> Y) (x : X) (xs : list X),
                   map f (snoc xs x) = snoc (map f xs) (f x).
Proof.
  intros X Y f x xs. induction xs.
  reflexivity. 
...
Proof.
  ...
  reflexivity. 
  simpl. rewrite <- IHl. rewrite -> map_snoc.
  reflexivity. 
Qed.
...
Proof.
  ...
  unfold override. rewrite -> H0. rewrite H. reflexivity. 
Qed.
...
(** Write down a theorem in Coq stating that [fold_map] is correct,
    and prove it. *)
Theorem fold_map_equiv : forall {X Y} (f : X -> Y) (l : list X),
